...
uint64_t UIRN_RUN(char const *s, unsigned int count) {
    return count ? (UIRN_RUN(s, count - 1) ^ s[count - 1]) * 16777619u : 2166136261u;
}

const uint64_t UIRN(char const *s, const unsigned int count) {
    return count ? (UIRN(s, count - 1) ^ s[count - 1]) * 16777619u : 2166136261u;
}

#define GENHASH(X) UIRN_RUN(X, strlen(X))
#define SETHASH(X) UIRN(X, strlen(X))